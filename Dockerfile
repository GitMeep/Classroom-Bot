...
WORKDIR /deps/
RUN apt update && apt install -y apt-utils git openssl libssl-dev zlib1g make g++ wget cmake
...
FROM ubuntu:21.04
WORKDIR /classroombot/
RUN apt update && apt install -y libcrypto++8 libcurl4 libicu67 libssl1.1
