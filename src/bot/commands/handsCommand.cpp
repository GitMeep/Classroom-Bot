...
const std::string handsPickMenuId = "hands_pick_user";
...
HandsCommand::HandsCommand() :
  Command::Command() {
  {
    ...
    dpp::command_option optionPickUserOptionUser(dpp::co_user, Localization::getString("hand_cmd_pick_option_user"), Localization::getString("hand_cmd_pick_option_user_desc"));
    ...
    for (const auto& lang : Localization::getLanguages()) {
      ...
      addLocalizationIfExists(optionPickUserOptionUser, currentLangCode, "hand_cmd_pick_option_user", "hand_cmd_pick_option_user_desc");
    ...
    pickCommand
        ...
        .add_option(optionPickUser
                        .add_option(optionPickUserOptionUser))
  ...
  // hand pick menu
  m_Spec.selectMenuIds.emplace_back(handsPickMenuId);
...
void HandsCommand::command(const CommandContext& ctx) {
  ...
  else if (commandName == Localization::getString("hand_cmd_lower"))
    lowerCmd(ctx);
  else if (commandName == Localization::getString("hand_cmd_hands"))
    handsCmd(ctx);
  else if (commandName == Localization::getString("hand_cmd_pick"))
    pickCmd(ctx);
  else {
...
void HandsCommand::selectClick(const CommandContext& ctx) {
  ...
  if (menuId == handsPickMenuId) pickFromList(ctx);
  else {
...
dpp::message pickMessage(const CommandContext& ctx, const dpp::snowflake& userId, bool wasRaised = false) {
  dpp::message msg(
      fmt::format(
...
void HandsCommand::pickCmd(const CommandContext& ctx) {
  ...
  if (subCommand == Localization::getString("hand_cmd_pick_option_next"))
    pickNext(ctx);
  else if (subCommand == Localization::getString("hand_cmd_pick_option_list"))
    pickList(ctx);
  else if (subCommand == Localization::getString("hand_cmd_pick_option_user"))
    pickUser(ctx);
  else if (subCommand == Localization::getString("hand_cmd_pick_option_random"))
    pickRandom(ctx);
  else if (subCommand == Localization::getString("hand_cmd_pick_option_clear"))
    pickClear(ctx);
  else {
...
void HandsCommand::pickNext(const CommandContext& ctx) {
  ...
  ctx.replyUnlocalized(pickMessage(ctx, userId, true));
}
...
void HandsCommand::pickUser(const CommandContext& ctx) {
  ...
  if (ci.options[0].options.size() == 0) {
    ctx.replyLocalized("hand_cmd_pick_option_user_no_user");
    return;
  }
  ...
  HandRepo::lower(ctx.channelId(), userId);
  ctx.replyUnlocalized(pickMessage(ctx, userId, wasRaised(userId, hands)));
}
...
void HandsCommand::pickFromList(const CommandContext& ctx) {
  ...
  dpp::snowflake selectedUserId = std::stoul(event.values[0]);

  // TODO: Figure out how the f to delete the original response with the dropdown menu without upsetting discord
  ctx.replyUnlocalized(pickMessage(ctx, selectedUserId, true));
...
void HandsCommand::pickUserContext(const CommandContext& ctx) {
  ...
  ctx.replyUnlocalized(pickMessage(ctx, selectedUser, wasRaised(selectedUser, hands)));
